...
function RetrospectivePanel({ hypothesisHistory, sessionSummary }: RetrospectivePanelProps) {
  ...
  return (
    <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-md p-4 mb-4">
        ...
        <div className="text-sm text-emerald-900 leading-relaxed">
          <ReactMarkdown
            components={{
              ...
              code({ children }) {
                return (
                  <code className="font-mono text-xs bg-emerald-100 border border-emerald-300 rounded px-1 py-0.5">
                    {children}
                  </code>
                );
              },
      ...
      {/* AI-generated content footnote */}
      <p className="mt-4 pt-3 border-t border-[var(--border-subtle)] text-[11px] italic text-[var(--text-muted)]">
        This summary was generated by AI. Use it as a starting point for reflection, not as a definitive record.
      </p>
    </div>
  );
}
...
function ChatHistory({ conversationHistory, isLoading, chatEndRef }: ChatHistoryProps) {
  const firstTutorIndex = conversationHistory.findIndex((m) => m.role === "tutor");
    ...
    <div className="flex-1 overflow-y-auto space-y-3 pr-1">
      ...
      {conversationHistory.map((msg, i) => (
        ...
        >
          ...
          {/* One-time AI disclaimer — shown only below the first tutor message */}
          {i === firstTutorIndex && (
            <p className="mt-1 text-[11px] italic text-[var(--text-muted)] max-w-[85%]">
              Keep in mind: I&apos;m an AI and I can make mistakes. If something I say doesn&apos;t seem right, trust your instincts and ask me to clarify.
            </p>
          )}
        </div>
      ))}
  ...
  return (
    <div className="flex flex-col gap-4 h-full" style={{ width: "50%" }}>
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className={isFixPhase ? "ring-1 ring-[var(--accent)] rounded-md overflow-hidden" : "flex-1 flex flex-col"}>
          <CodeEditor
            value={code}
            onChange={editable ? setCode : () => {}}
            readOnly={!editable}
            height={isFixPhase ? "auto" : "100%"}
          />
        </div>
      </div>
          ...
          <div className="flex gap-6 h-[calc(100vh-120px)]">
            ...
            <div className="flex flex-col" style={{ width: "50%" }}>
              ...
              <p className="mt-2 text-[11px] italic text-[var(--text-muted)]">
                Powered by Claude (AI) · AI responses may contain mistakes — always verify your understanding.
              </p>
            </div>
          </div>
        )}
